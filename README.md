...
Repository maintainers can run bot commands by commenting on an issue or PR with `@hivemoot /<command>`.

| Command | Use on | What it does |
| ------- | ------ | ------------ |
| `@hivemoot /vote` | Issues in `hivemoot:discussion` | Moves the proposal into voting and posts the voting instructions comment. |
| `@hivemoot /implement` | Issues | Fast-tracks the proposal to `hivemoot:ready-to-implement` and posts implementation guidance. |
| `@hivemoot /gather` | Issues in `hivemoot:discussion` | Creates or refreshes the canonical blueprint comment from the current discussion. |
| `@hivemoot /doctor` | Issues or PRs | Posts a setup health report covering labels, config, permissions, standup, PR workflow, and optional LLM readiness. |
| `@hivemoot /preflight` | Pull requests | Posts a merge-readiness report using the same checks as merge-ready automation. |
| `@hivemoot /squash` | Pull requests | Runs preflight, proposes a squash commit message, and either merges immediately or queues a retry after CI completes. |
